...
class MaskedTextDataset(Dataset):
	def __init__(self, text_data, tokenizer, mask_fraction=0.1):
...
def get_dataloader(text: np.ndarray, tokenizer, **kwargs):
	"""
	# get_dataloader
	@params
		text: (np.ndarray)
	...
	train_ds = MaskedTextDataset(text[:total_train_size], tokenizer, kwargs.get("mask_fraction",0.1))
	val_ds = MaskedTextDataset(text[total_train_size:], tokenizer, kwargs.get("mask_fraction",0.1))
